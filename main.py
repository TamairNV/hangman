...
def runGuess(letters,input):
    for letter in letters:
        if letter.letter == input:
            letter.displayedLetter = input

def displayLetters(letters,window):
    for letter in letters:
        letter.displayLetter(window)

def isWon(letters):
    for letter in letters:
        if letter.displayedLetter != letter.letter:
            return False
    return True
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and not gameOver:
            if event.key == pygame.K_RETURN and not (currentInputLetter  in guessedLetters or currentInputLetter == "_"):
                ...
                if currentInputLetter not in randomWord:
                    guessNum += 1
            ...
            if chr(event.key).lower() in alpha:
                currentInputLetter = chr(event.key)
